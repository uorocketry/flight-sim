...
E = (A**2-B**2)/(A**2)										# earth eccentricity
...
class Environment:
	...
	def geodetic_to_ecef(self, lat:float, long:float, alt:float) -> tuple[float, float, float]:
		...
		n = A**2/(A**2*math.cos(phi)**2 + B**2*math.sin(phi))**0.5
		print(n)
	...
	def g(self, lat:float, long:float, alt:float) -> float:
		...
		g = G_E * ((1+K*math.sin(math.radians(lat))**2)/math.sqrt(1-E**2))
...
if __name__ == "__main__":
	...
	print('x:{}, y:{}, z:{}'.format(round(ecef[0]/1000), round(ecef[1]/1000), round(ecef[2]/1000)))
